        ...
        crate::providers::next_sse_line(&mut lines, &mut deadline, stream_timeout).await?
    {
        ...
        match parsed_event.as_str() {
            ...
            "response.function_call_arguments.delta" => {
                ...
                let delta_str = if let Some(s) = parsed["delta"].as_str() {
                    s.to_string()
                ...
                if !delta_str.is_empty() {
                    ...
                    if let Some(acc) = acc {
                        acc.arguments.push_str(&delta_str);
                    }
                }
            }
...
\n";
            ...
            assert!(
                matches!(&resp.message.content[0], ContentBlock::Thinking { thinking, .. } if thinking == "\n\nFirst part\n\nSecond part")
            );
        })
    }
